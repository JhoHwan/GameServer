...
dtNavMesh* NavMeshLoader::LoadNavMeshFromBin(const std::filesystem::path& path)
{
    ...
    if (dtStatusFailed(status))
    {
        ...
        LOG_ERROR(Default, " - orig: [{}, {}, {}]", header.params.orig[0], header.params.orig[1], header.params.orig[2]);
        LOG_ERROR(Default, " - tileWidth: {}, tileHeight: {}", header.params.tileWidth, header.params.tileHeight);
        LOG_ERROR(Default, " - maxTiles: {}, maxPolys: {}", header.params.maxTiles, header.params.maxPolys);
        dtFreeNavMesh(mesh);
        return nullptr;
    }
