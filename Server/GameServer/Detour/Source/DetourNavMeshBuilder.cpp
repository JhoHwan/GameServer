...
bool dtCreateNavMeshData(dtNavMeshCreateParams* params, unsigned char** outData, int* outDataSize)
{
	...
	auto setHeaderShort = [](const int value, unsigned short& headerVal, const char* text, bool& bAllValuesSet)
		{
			dtAssert(value >= 0);
	...
	setHeaderShort(params->tileLayer, header->layer, "tile layer", bAllValuesSet);
	setHeaderShort(totPolyCount, header->polyCount, "poly count", bAllValuesSet);
	setHeaderShort(totVertCount, header->vertCount, "vert count", bAllValuesSet);
	setHeaderShort(maxLinkCount, header->maxLinkCount, "max link count", bAllValuesSet);
	dtVcopy(header->bmin, params->bmin);
	dtVcopy(header->bmax, params->bmax);
	setHeaderShort(params->polyCount, header->detailMeshCount, "detail mesh count", bAllValuesSet);
	setHeaderShort(uniqueDetailVertCount, header->detailVertCount, "detail vert count", bAllValuesSet);
	setHeaderShort(detailTriCount, header->detailTriCount, "detail tri count", bAllValuesSet);
	setHeaderShort(params->polyCount, header->offMeshBase, "off mesh base", bAllValuesSet);
	setHeaderShort(storedOffMeshConCount, header->offMeshConCount, "off mesh connection count", bAllValuesSet);
	setHeaderShort(params->buildBvTree ? params->polyCount * 2 : 0, header->bvNodeCount, "bv node count", bAllValuesSet);

	//@UE BEGIN
#if WITH_NAVMESH_SEGMENT_LINKS
	setHeaderShort(firstSegPoly, header->offMeshSegPolyBase, "off mesh seg poly base", bAllValuesSet);
	setHeaderShort(firstSegVert, header->offMeshSegVertBase, "off mesh seg vert base", bAllValuesSet);
	setHeaderShort(storedOffMeshSegCount, header->offMeshSegConCount, "off mesh seg con count", bAllValuesSet);
#endif // WITH_NAVMESH_SEGMENT_LINKS

#if WITH_NAVMESH_CLUSTER_LINKS
	setHeaderShort(params->clusterCount, header->clusterCount, "cluster count", bAllValuesSet);
#endif // WITH_NAVMESH_CLUSTER_LINKS
	//@UE END
